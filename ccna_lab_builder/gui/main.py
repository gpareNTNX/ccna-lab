...
class MainWindow(ttk.Frame):
    ...
    def _build(self):
        ...
        for widget, text in [
            (self.t_conn, " EVE-NG "), (self.t_images, " IOS IMAGES "),
            (self.t_master, " MASTER LAB "), (self.t_labs, " TRAINING LABS "),
    ...
    def _safe_run(self, func):
        try:
            ...
            self.after(0, lambda: messagebox.showerror("CCNA Lab Builder", str(exc)))

    @staticmethod
    def field(parent, label, row, value="", secret=False):
        ttk.Label(parent, text=label, width=22).grid(row=row, column=0, sticky="w", pady=7)
    ...
    def _images_tab(self):
        f = ttk.Frame(self.t_images, padding=30); f.pack(fill="both", expand=True)
        ttk.Label(f, text="Cisco virtual images", font=("Helvetica", 17, "bold")).pack(anchor="w", pady=(0, 14))
        self.router_label = ttk.Label(f, text="IOSv L3: not selected"); self.router_label.pack(anchor="w", pady=5)
        ttk.Button(f, text="SELECT IOSv", command=lambda: self.pick_image(False)).pack(anchor="w", pady=5)
        self.switch_label = ttk.Label(f, text="IOSvL2: not selected"); self.switch_label.pack(anchor="w", pady=(15, 5))
        ttk.Button(f, text="SELECT IOSvL2", command=lambda: self.pick_image(True)).pack(anchor="w", pady=5)
        ttk.Separator(f).pack(fill="x", pady=20)
        ttk.Button(f, text="INSTALL SELECTED IMAGES", command=lambda: self.bg(self.install_images)).pack(anchor="w")
        ttk.Button(f, text="SCAN INSTALLED IMAGES", command=lambda: self.bg(self.scan_images)).pack(anchor="w", pady=10)

    def pick_image(self, l2):
        path = filedialog.askopenfilename(filetypes=[("QEMU image", "*.qcow2 *.qcow *.img"), ("All files", "*.*")])
        if not path:
            return
        try:
            ...
            if l2:
                ...
                self.switch_label.configure(text=f"IOSvL2: {path}\nEVE folder: {info['folder']}")
            else:
                self.router_image = (path, info)
                self.router_label.configure(text=f"IOSv: {path}\nEVE folder: {info['folder']}")
        except Exception as exc:
    ...
    def install_images(self):
        ...
        if self.router_image:
            self.router_folder = install_image(self.ssh, self.router_image[0], self.router_image[1], self.log)
        if self.switch_image:
            self.switch_folder = install_image(self.ssh, self.switch_image[0], self.switch_image[1], self.log)
    ...
    def _master_tab(self):
        f = ttk.Frame(self.t_master, padding=30); f.pack(fill="both", expand=True)
        ...
        self.experimental = tk.BooleanVar(value=self.settings.data["compatibility"]["experimental_cabling"])
        ttk.Checkbutton(
            f,
            text="Enable experimental API cabling (version-dependent)",
            variable=self.experimental,
        ).grid(row=2, column=1, sticky="w", pady=8)
        ttk.Button(f, text="BUILD MASTER LAB", command=lambda: self.bg(self.build_master)).grid(row=3, column=1, sticky="e", pady=18)
        ttk.Button(f, text="START ALL", command=lambda: self.bg(lambda: self.lab_action("start"))).grid(row=4, column=1, sticky="e", pady=4)
        ttk.Button(f, text="STOP ALL", command=lambda: self.bg(lambda: self.lab_action("stop"))).grid(row=5, column=1, sticky="e", pady=4)
        ttk.Button(f, text="WIPE ALL", command=self.confirm_wipe).grid(row=6, column=1, sticky="e", pady=4)

    def _selected_images(self):
        if not self.router_folder:
            if self.router_image:
                self.router_folder = self.router_image[1]["folder"]
        if not self.switch_folder:
            if self.switch_image:
    ...
    def build_master(self):
        if not self.api:
            raise RuntimeError("Connect to EVE-NG first.")
        r, s = self._selected_images()
        builder = LabBuilder(self.api, self.log)
        lab = builder.create(
            self.folder.get().strip(), self.master_name.get().strip(),
            r, s, cable=self.experimental.get()
        )
        self.settings.data["lab"].update({"folder": self.folder.get().strip(), "master_name": self.master_name.get().strip()})
    ...
    def confirm_wipe(self):
        if messagebox.askyesno("Wipe lab", "This removes user configuration from all nodes. Continue?"):
            self.bg(lambda: self.lab_action("wipe"))

    def _labs_tab(self):
        outer = ttk.Frame(self.t_labs, padding=22); outer.pack(fill="both", expand=True)
        left = ttk.Frame(outer); left.pack(side="left", fill="y")
        right = ttk.Frame(outer); right.pack(side="left", fill="both", expand=True, padx=(20, 0))
        self.lab_list = tk.Listbox(left, width=34, height=25)
        for s in self.catalog.all():
            self.lab_list.insert("end", f"{s['id']} - {s['name']}")
        self.lab_list.pack(fill="y", expand=True)
        self.lab_list.bind("<<ListboxSelect>>", self.select_scenario)
        self.scenario_title = ttk.Label(right, text="Select a lab", font=("Helvetica", 17, "bold"))
        self.scenario_title.pack(anchor="w")
        self.scenario_meta = ttk.Label(right, text=""); self.scenario_meta.pack(anchor="w", pady=5)
        self.scenario_text = tk.Text(right, height=20, wrap="word", state="disabled")
        self.scenario_text.pack(fill="both", expand=True, pady=10)
        ttk.Button(right, text="CREATE FRESH SCENARIO LAB", command=lambda: self.bg(self.create_scenario_lab)).pack(anchor="e")
    ...
    def select_scenario(self, _event=None):
        ...
        self.scenario_title.configure(text=f"{s['id']} — {s['name']}")
        self.scenario_meta.configure(text=f"{s['domain']} • {s['difficulty']} • {s['minutes']} min")
        body = s["objective"] + "\n\nTasks:\n" + "\n".join(f"{i+1}. {x}" for i, x in enumerate(s["tasks"]))
    ...
    def create_scenario_lab(self):
        ...
        name = f"CCNA-{scenario['id']}-{scenario['name'].upper().replace(' ', '-')}"
        lab = LabBuilder(self.api, self.log).create(
            self.folder.get().strip(), name, r, s, cable=self.experimental.get()
        )
        self.log("Scenario lab created: " + lab)

    def _validator_tab(self):
        f = ttk.Frame(self.t_validate, padding=22); f.pack(fill="both", expand=True)
        ttk.Label(f, text="Live validator", font=("Helvetica", 17, "bold")).pack(anchor="w")
        ttk.Label(
            f,
            text="Starts with the currently selected scenario. Nodes must already be running. "
                 "Live mode tunnels to each EVE console through SSH.",
            wraplength=950
        ...
        ttk.Button(buttons, text="VALIDATE LIVE", command=lambda: self.bg(self.validate_live)).pack(side="left")
        ttk.Button(buttons, text="CLEAR", command=lambda: self.validation_output.delete("1.0", "end")).pack(side="left", padx=8)
    ...
    def validate_live(self):
        ...
        results = LiveValidator(self.api, self.ssh, self.log).validate(
            self.validation_lab.get().strip(), self.current_scenario
        )
        score = Validator.score(results)
        text = [f"Score: {score}%\n"]
        for r in results:
            text.append(f"{'PASS' if r.passed else 'FAIL'} | {r.node} | {r.command}")
            if r.missing:
                text.append("  Missing: " + ", ".join(r.missing))
        payload = "\n".join(text)
        self.after(0, lambda: (self.validation_output.delete("1.0", "end"), self.validation_output.insert("1.0", payload)))
